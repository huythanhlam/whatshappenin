// End-to-end ingest of sweatpals.com through the app's real pipeline: the
// 'sweatpals' source parser (obtain) → persistEvents (dedupe, tag, geocode,
// insert). Run against the app's own data layer so it exercises the same code
// the site uses.
//
//   npx tsx scripts/ingest-sweatpals.ts
//
...
// its `sources` row lives in migration 045, which is past the legacy PGlite
...
import { fetchSweatpalsEvents } from '@/lib/sources/sweatpals'
...
const SOURCE = 'crawl:sweatpals-com'
const URL = 'https://sweatpals.com/discover'
...
async function main() {
  ...
  console.log(`\n[1/4] Obtaining events for ${CITY.name}, ${CITY.state} (day-windowed sweep of the public API)...`)
  const t0 = Date.now()
  const events = await fetchSweatpalsEvents(URL, SOURCE, { cityName: CITY.name, state: CITY.state })
  console.log(`  Obtained ${events.length} events in ${((Date.now() - t0) / 1000).toFixed(1)}s (deduped by series+occurrence).`)
  if (events.length === 0) {
  ...
  const withEnd = events.filter(e => e.end_time).length
  const free = events.filter(e => e.is_free).length
  console.log(
    `  ${withVenue}/${events.length} have a venue, ${withAddress} an address, ${withEnd} a trusted end time, ${free} are free;` +
